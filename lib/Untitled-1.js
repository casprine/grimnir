...
// Set prompt as green
prompt.message = colors.green('Project');
...
module.exports = (args, options, logger) => {
  ...
  const localPath = `${process.cwd()}/${args.name.toLowerCase()}`;
  ...
  if (fs.existsSync(localPath)) {
    logger.info(`Sorry but ${colors.green(args.name)} is already a directory name ☹️\n`);
  ...
  if (fs.existsSync(templatePath)) {
    logger.info(`Creating a new ${args.templateName} app in ${colors.green(localPath)}'\n`);
  ...
  logger.info('Please fill the following project details....\n');
